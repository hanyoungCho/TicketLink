...
*)
  ...
  private
    ...
    function MakeSendDataCard(AInfo: TCardSendInfoDM): AnsiString;
    function MakeSendDataCash(AInfo: TCardSendInfoDM): AnsiString;
    function MakeRecvDataCard(ARecvData: AnsiString; const AOfflinePG: Boolean): TCardRecvInfoDM;
    function MakeRecvDataCash(ARecvData: AnsiString): TCardRecvInfoDM;
    ...
    function DoPosDownload(ATerminalID, ABizNo: AnsiString): TCardRecvInfoDM; // 가맹점 다운로드 (멀티 TID 사용매장에서 사용함)
  protected
  public
    constructor Create; override;
    destructor Destroy; override;
    ...
    function CallTransCancel: Boolean; override;
...
begin
  FVanCode := '';
  FLog := TLog.Create;
  FLog.Write(ltInfo, ['VanDaemonModule', '2019.01.23.1']);
end;

destructor TVanDaemonModule.Destroy;
begin
  FLog.Write(ltInfo, ['VanDaemonModule', 'Destroy']);
  FLog.Free;
  if MainVan <> nil then
    MainVan.Free;
  inherited;
...
begin
  FLog.Write(ltInfo, ['VanDaemonModule 전체 설정 적용', FVanCode]);
  if MainVan <> nil then
    FreeAndNil(MainVan);
...
begin
  Result := False;
  FLog.Write(ltBegin, ['결제대기취소', FVanCode]);
  try
    if (MainVan = nil) then
      Exit;
    Result := MainVan.CallTransCancel;
  except
    on E: Exception do
      FLog.Write(ltError, ['결제대기취소', E.Message]);
  end;
end;
...
begin
  inherited;
  FSmtSndRcvVCAT := TSmtSndRcvVCAT.Create(nil);
end;
...
begin
  ...
  // 승인 호출
  Ret := FSmtSndRcvVCAT.PosToVCATSndRcv('127.0.0.1', 13855, 60);
  if Ret = 1 then
  begin
...
begin
  Result := False;
  if FSmtSndRcvVCAT.SetTRData(AIndex, AValue) = 1 then
  begin
    ARetMsg := '정상';
    Result := True;
  end
...
begin
  Result := FSmtSndRcvVCAT.GetResDataCount(1, AIndex);
end;
...
begin
  ...
  Result := Result + 'N';
  // 카드삽입대기창 숨김 여부
  Result := Result + IfThen(HideLegacyForm, 'Y', 'N'); //Y:숨김, N:표시
  // 예비필드
  Result := Result + Format('%-4.4s', ['']) + FS;
...
function TVanKcpDaemon.MakeRecvDataCard(ARecvData: AnsiString; const AOfflinePG: Boolean): TCardRecvInfoDM;
begin
  // ETX까지 자른다.
  ...
  begin
    ...
    // 거래고유번호
    if AOfflinePG then
      Result.TransNo := StringSearch(ARecvData, FS, 34) //34:SALE_NO(V:16)
    else
      Result.TransNo := StringSearch(ARecvData, FS, 13); //TX_NO(V:12)
    // 총거래금액
...
function TVanKcpDaemon.CallTransCancel: Boolean;
type
  TKCPDaemonTransCancel = function: Integer; stdcall;
var
  Ret: Integer;
  Exec: TKCPDaemonTransCancel;
begin
  Result := False;
  @Exec := GetProcAddress(FDLLHandle, 'KCP_TransCancel');
...
begin
  ...
  if DLLExecTrans(SendData, RecvData) then
    Result := MakeRecvDataCard(RecvData, (AInfo.Reserved1 = 'PG'))
  else
  begin
    Result.Result := False;
...
begin
  ...
  begin
    ...
    // BIN 코드는 6자리 -> 8자리로 변경 (LSW: 20231130)
    Result.CardBinNo := StringSearch(RecvData, FS, 8);
  end
  else
  begin
    Result.Result := False;
...
begin
  ...
  begin
    ...
    if (ARespCode = '0000') then
      Result := (StringSearch(ARecvData, FS, 6) = 'Y');
  end;
